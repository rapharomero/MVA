...
/// Constraints : i>=win i<width - win , j >=win and j < height-win
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    if( !(i1>=win  && i1<im1.width() - win && j1>=win && j1<im1.height()-win)){
            cout<< "Unable to compute correl : Some points are not inside image 1"<< endl;
    }
    if( !(i2>=win  && i2<im2.width() - win && j2>=win && j2<im2.height()-win)){
            cout<< "Unable to compute correl : Some points are not inside image 2"<< endl;
    }
    float dist=0.0f;
    float var1 = 0.0f;
    float var2 = 0.0f;
    for(int shiftx = -win; shiftx <= +win ;shiftx++){
        for(int shifty = -win; shifty <= +win ; shifty++){
            dist += (im1(i1+shiftx,j1+shifty)-m1)*(im2(i2+shiftx,j2+shifty)-m2);
            var1 += (im1(i1+shiftx,j1+shifty)-m1)*(im1(i1+shiftx,j1+shifty)-m1);
            var2 += (im2(i2+shiftx,j2+shifty)-m2)*(im2(i2+shiftx,j2+shifty)-m2);
        }
    }
    dist = dist/(EPS+sqrt(var1)*sqrt(var2));
    return dist;
}
...
{
    float s=0.0f;
    if(!(i>=win  && i<im.width() - win )){
        cout<< "Unable to compute sum : x = "<< i << " out of bound"<< endl;
    }
    if(!(j>=win && j<im.height()-win)){
            cout<< "Unable to compute sum : y = " << j << " out of bound"<< endl;
    }
    for(int shiftx = -win; shiftx <= +win ;shiftx++){
        for(int shifty = -win; shifty <= +win ; shifty++){
            s += im(i+shiftx,j+shifty);
        }
    }
    return s;
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<im1.height() && y+win<im2.height(); y++)
        for(int x=win; x+win<im1.width(); x++) {
            float dOpt = 0.0f;
            float nccOpt = ccorrel(im1,x,y,
                                   im2,x,y);
            // Search for disparity that gives the best NCC at point(x,y)
            // Define disparity window in which to search for best disparity
            float d1 = dMin;
            float d2 = dMax;
            for(int shift = 0; d1+shift < d2 ; shift++){
                float d = d1+shift;
                if(x+d>=win && x+d+win<im2.width()){
                    float ncc = ccorrel(im1,x,y,
                                        im2,x+d,y);
                    if(ncc > nccOpt){
                        nccOpt = ncc;
                        dOpt = d;
    ...
    while(! Q.empty()) {
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                // ------------- TODO -------------
                // Find disparity among dP -1,dP and dP+1 that has the highest ncc.
                float disparity = s.d;
                //Initialize best disparity
                float dOpt = disparity;
                float nccOpt = s.ncc;
                for(int shift = -1; shift <= 1;shift++){
                    float d = disparity + shift;
                    if(x+d>=win && x+d+win<im2.width()){
                        float ncc = ccorrel(im1,x,y,
                                            im2,x+d,y);
                        if(ncc> nccOpt){
